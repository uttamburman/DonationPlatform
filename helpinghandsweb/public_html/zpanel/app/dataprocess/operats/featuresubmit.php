		...
		if (isset($_SESSION["eid"])) {
			...
			$sno = 10000001;
	  		$sql="";
				...
				$sid=$sno;
				$resx2=$connection->selectTable("SELECT count(*) as count from tempsubmit where segment='featured';");
				if ($resx2->num_rows > 0) 
				{
					while($row = $resx2->fetch_assoc()) 
						{
							...
							if($legendcount<18){								
								$resx3=$connection->selectTable("SELECT count(*) as count from tempsubmit where segment='featured' && eid='".$eid."';");
							if ($resx3->num_rows > 0) 
							{					
								while($row1 = $resx3->fetch_assoc()) 
								{
									...
									if($submitcount<6){			
													$stmt = $mysqli->prepare("insert into tempsubmit values(?,?,?,?,'featured');");
													$stmt->bind_param('ssss', $sno , $aid , $eid , $dte);
													
													$stmt1=$mysqli->prepare("insert into allsubmit values(?,?,?,?,'featured' ,'submitted');");
